...
export function normalizeCaseEvaluation(
  value: unknown,
  fallbackId: string,
...
export function normalizeComparatorResult(value: unknown, cases: EvalCase[]): ComparatorResult {
  ...
  const caseDecisions = cases.map((evalCase, index) => {
    ...
    const winner =
      raw?.winner === "A" || raw?.winner === "B" || raw?.winner === "tie" ? raw.winner : "tie";
    return {
...
export function countAttemptResults(attempts: { accepted: boolean }[]): {
